...
public class DataService {
    ...
    public void addAdminAndUser() {
        List<Authority> authoritiesAdmin = authorityRepository.findAllById(
            Arrays.asList(AuthoritiesConstants.USER, AuthoritiesConstants.ADMIN)
        );
        for (int i = 1; i <= 5; i++) {
            User user = new User();
            ...
            user.setActivated(true);
            user.setAuthorities(new HashSet<>(authoritiesAdmin));
